...
class ReporteAdminController extends CRUDController {
    ...
    public function detalladoAction() {
        ...
        $formTipos = $this->createForm(new RDetalladoTiposFormType($this->container));
        $formTotalTipos = $this->createForm(new RDetalladoTotalTiposFormType($this->container));
        $formGestor = $this->createForm(new RDetalladoGestorFormType($this->container));
        $formAprovechables = $this->createForm(new RDetalladoAprovechablesFormType($this->container));
        $formFechas = $this->createForm(new RDetalladoFechasFormType($this->container));
                ...
                if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                    ...
                    try {
                        ...
                        if ($this->isXmlHttpRequest()) {
                            return $this->render("BackendBundle:Reportes:Detallado/Tipos/template_grafica.html.twig", array(
                            ...
                            if ($this->get('request')->get('pdf')) {
                                $html = $this->renderView("BackendBundle:Reportes:Detallado/Tipos/template_grafica_pdf.html.twig", array(
                                ...
                                foreach ($datos['series'] as $tipoResiduo) {
                                    ...
                                    $sumaTotal = $sumaTotal + $tipoResiduo['peso'];
                                    $peso = number_format($tipoResiduo['peso'], 3, ',', '.');
                                    $activeSheet->setCellValue('B' . $fila, $peso);
                                    $fila++;
                                }
                                $activeSheet->setCellValue('A' . $fila, "Total");
                                $activeSheet->setCellValue('B' . $fila, number_format($sumaTotal, 3, ',', '.'));
            ...
            if ($this->getRequest()->request->has('r_detallado_total_tipos_form')) {
                ...
                if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                    ...
                    try {
                        ...
                        $titulo = $datos['titulo'];
                        if ($this->isXmlHttpRequest()) {
                            return $this->render("BackendBundle:Reportes:Detallado/TotalTipos/template_grafica.html.twig", array(
                                        'nombre_grafica' => $nombre_grafica,
                                        'datos' => $datos['series'],
                                        'titulo' => $titulo,
                            ));
                            ...
                            if ($this->get('request')->get('pdf')) {
                                $html = $this->renderView("BackendBundle:Reportes:Detallado/TotalTipos/template_grafica_pdf.html.twig", array(
                                    'base_dir' => $this->container->get('kernel')->getRootDir() . '/../web/',
                                    'nombre_grafica' => $nombre_grafica,
                                    'datos' => $datos['series'],
                                    'graficaBase64' => $base64
                                ...
                                $activeSheet->setCellValue('A' . $fila, 'Total');
                                $activeSheet->setCellValue('B' . $fila, number_format($totalPeso, 3, ',', '.'));
                                $activeSheet->setCellValue('C' . $fila, '100');
                                $objDrawing = new \PHPExcel_Worksheet_Drawing();
                                $objDrawing->setName('Grafica');
                                $objDrawing->setDescription('Grafica');
                                $objDrawing->setPath($imagenGrafica);
            ...
            if ($this->getRequest()->request->has('r_detallado_gestor_form')) {
                ...
                if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                    ...
                    try {
                        ...
                        $titulo = $datos['titulo'];
                        if ($this->isXmlHttpRequest()) {
                            return $this->render("BackendBundle:Reportes:Detallado/Gestor/template_grafica.html.twig", array(
                                        'nombre_grafica' => $nombre_grafica,
                                        'datos' => $datos['series'],
                                        'titulo' => $titulo,
                            ));
                            ...
                            if ($this->get('request')->get('pdf')) {
                                $html = $this->renderView("BackendBundle:Reportes:Detallado/Gestor/template_grafica_pdf.html.twig", array(
                                    'base_dir' => $this->container->get('kernel')->getRootDir() . '/../web/',
                                    'nombre_grafica' => $nombre_grafica,
                                    'datos' => $datos['series'],
                                    'graficaBase64' => $base64
            ...
            if ($this->getRequest()->request->has('r_detallado_fechas_form')) {
                ...
                if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                    ...
                    try {
                        ...
                        if ($this->isXmlHttpRequest()) {
                            return $this->render("BackendBundle:Reportes:Detallado/Mensual/template_grafica.html.twig", array(
                                        'nombre_grafica' => $nombre_grafica,
                                        'datos' => $datos['series'],
                            ...
                            if ($this->get('request')->get('pdf')) {
                                $html = $this->renderView("BackendBundle:Reportes:Detallado/Mensual/template_grafica_pdf.html.twig", array(
                                    'base_dir' => $this->container->get('kernel')->getRootDir() . '/../web/',
                                    'nombre_grafica' => $nombre_grafica,
                                    'datos' => $datos['series'],
                                    'graficaBase64' => $base64,
                                ...
                                $activeSheet->setCellValue($columna . '3', "Total");
                                ...
                                foreach ($datos['series'] as $fecha => $tipos) {
                                    ...
                                    foreach ($tipos as $tipo) {
                                        $sumaTotal = $sumaTotal + $tipo['peso'];
                                        $peso = number_format($tipo['peso'], 3, ',', '.');
                                        $activeSheet->setCellValue($columna . $fila, $peso);
                                        $columna++;
                                    }
                                    $activeSheet->setCellValue($columna . $fila, number_format($sumaTotal, 3, ',', '.'));
                                    $fila++;
                                }
                                $objDrawing = new \PHPExcel_Worksheet_Drawing();
                                $objDrawing->setName('Grafica');
            ...
            if ($this->getRequest()->request->has('r_detallado_aprovechables_form')) {
                ...
                if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                    ...
                    try {
                        ...
                        $titulo = $datos['titulo'];
                        if ($this->isXmlHttpRequest()) {
                            return $this->render("BackendBundle:Reportes:Detallado/Aprovechables/template_grafica.html.twig", array(
                                        'nombre_grafica' => $nombre_grafica,
                                        'datos' => $datos['series'],
                                        'titulo' => $titulo,
                            ));
                            ...
                            if ($this->get('request')->get('pdf')) {
                                $html = $this->renderView("BackendBundle:Reportes:Detallado/Aprovechables/template_grafica_pdf.html.twig", array(
                                    'base_dir' => $this->container->get('kernel')->getRootDir() . '/../web/',
                                    'nombre_grafica' => $nombre_grafica,
                                    'datos' => $datos['series'],
                                    'graficaBase64' => $base64
        ...
        return $this->render("BackendBundle:Reportes/Detallado:base_edit.html.twig", array(
                    'action' => 'create',
                    'formTipos' => $viewTipos,
    ...
    public function operativoAction() {
        ...
        if ($this->getRestMethod() == 'POST') {
            ...
            if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                ...
                try {
                    ...
                    $titulo = $datos['titulo'];
                    if ($this->isXmlHttpRequest()) {
                        return $this->render("BackendBundle:Reportes:Operativo/template_grafica.html.twig", array(
                                    'nombre_grafica' => $nombre_grafica,
                                    'datos' => $datos['series'],
                                    'titulo' => $titulo,
                        ));
                        ...
                        if ($this->get('request')->get('pdf')) {
                            $html = $this->renderView("BackendBundle:Reportes:Operativo/template_grafica_pdf.html.twig", array(
                                'base_dir' => $this->container->get('kernel')->getRootDir() . '/../web/',
                                'nombre_grafica' => $nombre_grafica,
                                'datos' => $datos['series'],
                                'graficaBase64' => $base64
        ...
        return $this->render("BackendBundle:Reportes:Operativo/base_edit.html.twig", array(
                    'action' => 'create',
                    'form' => $view,
                    'object' => $object,
        ));
    ...
    public function residuosDisposicionAction() {
        if (false === $this->admin->isGranted('CREATE')) {
            throw new AccessDeniedException();
        }
        ...
        if ($this->getRestMethod() == 'POST') {
            ...
            if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                ...
                try {
                    ...
                    $titulo = $datos['titulo'];
                    if ($this->isXmlHttpRequest()) {
                        return $this->render("BackendBundle:Reportes:ResiduosDisposicion/template_grafica.html.twig", array(
                                    'nombre_grafica' => $nombre_grafica,
                                    'datos' => $datos['series'],
                                    'titulo' => $titulo,
                        ));
                        ...
                        $base64 = $form->get('imagen_grafica')->getData();
                        if ($this->get('request')->get('pdf')) {
                            $html = $this->renderView("BackendBundle:Reportes:ResiduosDisposicion/template_grafica_pdf.html.twig", array(
                                'base_dir' => $this->container->get('kernel')->getRootDir() . '/../web/',
                                'nombre_grafica' => $nombre_grafica,
                                'datos' => $datos['series'],
                                'graficaBase64' => $base64
                            ...
                            $activeSheet->setCellValue('A' . $fila, 'Total');
                            $activeSheet->setCellValue('B' . $fila, $totalPeso);
                            $activeSheet->setCellValue('C' . $fila, 100);
                            $objDrawing = new \PHPExcel_Worksheet_Drawing();
                            $objDrawing->setName('Grafica');
                            $objDrawing->setDescription('Grafica');
        ...
        $view = $form->createView();
        return $this->render("BackendBundle:Reportes:ResiduosDisposicion/base_edit.html.twig", array(
                    'action' => 'create',
                    'form' => $view,
                    'object' => $object,
        ));
    ...
    public function checklistAction() {
        ...
        if ($this->getRestMethod() == 'POST') {
            ...
            if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                ...
                try {
                    ...
                    if ($this->isXmlHttpRequest()) {
                        return $this->render("BackendBundle:Reportes:Checklist/template_grafica.html.twig", array(
                                    'nombre_grafica' => $nombre_grafica,
                        ...
                        $base64 = $form->get('imagen_grafica')->getData();
                        if ($this->get('request')->get('pdf')) {
                            $html = $this->renderView("BackendBundle:Reportes:Checklist/template_grafica_pdf.html.twig", array(
                                'base_dir' => $this->container->get('kernel')->getRootDir() . '/../web/',
                                'nombre_grafica' => $nombre_grafica,
                                'cumplen' => $datosCumplen['series'],
                                'noCumplen' => $datosNoCumplen['series'],
        ...
        $view = $form->createView();
        return $this->render("BackendBundle:Reportes:Checklist/base_edit.html.twig", array(
                    'action' => 'create',
                    'form' => $view,
                    'object' => $object,
        ));
    ...
    public function capacitacionesAction() {
        ...
        if ($this->getRestMethod() == 'POST') {
            ...
            if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                ...
                try {
                    ...
                    if ($this->get('request')->get('pdf')) {
                        $html = $this->renderView("BackendBundle:Reportes:Capacitaciones/template_grafica_pdf.html.twig", array(
                            'base_dir' => $this->container->get('kernel')->getRootDir() . '/../web/',
                            'datos' => $datos['series'],
                            'titulo' => $datos['titulo'],
                        ));
        ...
        $view = $form->createView();
        return $this->render("BackendBundle:Reportes:Capacitaciones/base_edit.html.twig", array(
                    'action' => 'create',
                    'form' => $view,
                    'object' => $object,
        ));
    ...
    public function economicoAction() {
        if (false === $this->admin->isGranted('CREATE')) {
            throw new AccessDeniedException();
        }
        ...
        if ($this->getRestMethod() == 'POST') {
            if ($this->getRequest()->request->has('r_detallado_economico_form')) {
                ...
                if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                    ...
                    try {
                        ...
                        $titulo = $datos['titulo'];
                        if ($this->isXmlHttpRequest()) {
                            return $this->render("BackendBundle:Reportes:Economico/template_grafica.html.twig", array(
                                        'nombre_grafica' => $nombre_grafica,
                                        'datos' => $datos['series'],
                                        'titulo' => $titulo,
                            ));
                            ...
                            if ($this->get('request')->get('pdf')) {
                                $html = $this->renderView("BackendBundle:Reportes:Economico/template_grafica_pdf.html.twig", array(
                                    'base_dir' => $this->container->get('kernel')->getRootDir() . '/../web/',
                                    'nombre_grafica' => $nombre_grafica,
                                    'datos' => $datos['series'],
                                    'graficaBase64' => $base64
                                ...
                                $activeSheet
                                        ->setCellValue('B1', $datos['titulo'])
                                        ->setCellValue('A3', 'Tipo de residuo')
                                        ->setCellValue('B3', 'Precio');
                                $activeSheet->mergeCells('B1:F1');
                                $fila = 4;
                                $sumaTotal = 0;
                                foreach ($datos['series'] as $tipoResiduo) {
                                    $sumaTotal = $sumaTotal + $tipoResiduo['precio'];
                                    $activeSheet->setCellValue('A' . $fila, $tipoResiduo['nombre']);
        ...
        return $this->render("BackendBundle:Reportes:Economico/base_edit.html.twig", array(
                    'action' => 'create',
                    'form' => $viewEconomico,
                    'object' => $object,
        ));
    ...
    public function dataReporteResiduosDisposicion($data) {
        ...
        $series = array();
        $em = $this->getDoctrine()->getManager();
        $titulo = 'Tipos de residuos y sus disposiciones finales';
        if ($tipoResiduo) {
    ...
    public function getFooter() {
        return $this->get('kernel')->getRootDir() . "/../src/BackendBundle/Resources/views/Reportes/footer_pdf.html";
    }
